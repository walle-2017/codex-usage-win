...
Write-Host 'PASS: default layout and square themed taskbar UI contract is satisfied.'
 -or
    $windowProduction -match 'IDM_LAYOUT_COMPACT' -or
    $windowProduction -notmatch 'IDM_LAYOUT_DEFAULT') {
    throw 'Legacy Default naming must be removed; layout presets are Default and Minimal.'
}
if ($windowProduction -match '(?m)^const SEGMENT_COUNT:') {
    throw 'Obsolete Default-preset SEGMENT_COUNT constant must be removed.'
}
